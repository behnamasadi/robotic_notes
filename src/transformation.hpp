...
void transformPoints(cv::Mat rotation, cv::Mat translation,
  ...
  for (std::size_t i = 0; i < inputPoints.size(); i++) {
    cv::Point3d X =
        cv::Point3d(inputPoints[i].x, inputPoints[i].y, inputPoints[i].z);
    cv::Mat X_mat = cv::Mat(X);
    cv::Mat pointInCameraMatrix = rotation * X_mat + translation;
    cv::Point3d pointInCameraCoordinate =
        cv::Point3d(pointInCameraMatrix.at<double>(0, 0),
                    pointInCameraMatrix.at<double>(1, 0),
                    pointInCameraMatrix.at<double>(2, 0));
