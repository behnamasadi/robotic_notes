 ...
 *        d_ij    = X_j (-) X_i = log(X_i.inv * X_j)  // motion expectation
 * equation
 *
 *    - for the measurement factors:
 *        e_ik    = h(X_i, b_k) = X_i^-1 * b_k        // measurement expectation
 * equation
 *
 *
 *
 ...
 *
 *  Printing the prior state (before solving) and posterior state (after
 * solving), together with a ground-truth state defined by the simulator allows
 * for evaluating the quality of the estimates.
 *
...
int main() {
  ...
  MatrixY S; // sqrt Info
  vector<map<int, VectorY>> measurements(
      NUM_POSES); // y = measurements[pose_id][lmk_id]
  ...
  Matrix<double, NUM_STATES, 1>
      dX; // optimal update step for all the SAM problem
  ...
  for (int i = 0; i < NUM_POSES; ++i) {
    ...
    if (i <
        NUM_POSES -
            1) // do not make the last motion since we're done after 3rd pose
    {
  ...
  for (const auto &X : poses)
    cout << "pose  : " << X.translation().transpose() << " " << X.angle()
         << endl;
  ...
  for (int iteration = 0; iteration < MAX_ITER; ++iteration) {
     ...
     *       = W * (e * y.inv).log()
     *
     *  When `y` is provided as a local reference, then right-minus (-.) is
     * required,
     *
     *     r = W * (e (-.) y)
     *       = W * (y.inv * e).log()
     *
     *  Notice that if y = Identity() then local and global residuals are the
     * same.
     *
     *
     ...
     *     r = W * (poses[0] (.-) measurement) = log(poses[0] * Id.inv) =
     * poses[0].log()
     *
     *  Jacobian matrix :
     ...
     *
     *     with W = i and y = I. Since d(log(r))/d(r) = Jr_inv(r) for any r in
     * the Lie algebra, we have J_r_p0 = Jr_inv(log(p0))
     */
    ...
    r.segment<DoF>(row) =
        poses[0].lminus(SE2d::Identity(), J.block<DoF, DoF>(row, col)).coeffs();
    ...
    for (int i = 0; i < NUM_POSES; ++i) {
      ...
      for (const auto &k : pairs[i]) {
        ...
        // expectation
        e = Xi.inverse(J_ix_x).act(b, J_e_ix,
                                   J_e_b); // expected measurement = Xi.inv * bj
    ...
    // ATTENTION: Use QR factorization and column reordering for larger
    // problems!!
    ...
    // DEBUG INFO
    cout << "residual norm: " << std::scientific << r.norm()
         << ", step norm: " << dX.norm() << endl;
